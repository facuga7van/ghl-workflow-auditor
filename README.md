...
You get a findings report to paste into a ticket, and a JSON dump of every step of every workflow — in real execution order — meant to be handed to an AI coding agent.
...
The JSON carries an embedded `_readme` explaining its own structure, so you can hand it straight to an AI agent and it will know that `steps` is in real execution order (the `order` field GoHighLevel returns is unreliable), that fields are referenced by id, and that `inFlight` is people inside the workflow *right now*.
...
It also builds the **trigger graph**, including the implicit edges: workflow A adds a tag, workflow B triggers on it. Nothing in the GoHighLevel UI ever draws you that line.
